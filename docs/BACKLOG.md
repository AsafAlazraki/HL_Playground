...
| 86 | OPEN | FOUND 2026-09-10, verifying row 3 | Two counted figures disagree about the same tables. The removal control says "Remove the **53** example tables"; the tables gallery header says "**51** Tables" and the nav rail says "Data 53". Every figure in this app is counted rather than typed, which is the whole point of it, so two of them differing by two is either a filter nobody stated or a count that is wrong. | hours | Measured in the running app 2026-09-10: gallery shows 51 cards and 51 chips; `exampleOnSheet()` reports 53. Neither number is obviously the wrong one — the gallery may be excluding retired or join tables without saying so, which would make the HEADER the thing to fix rather than the control. |
