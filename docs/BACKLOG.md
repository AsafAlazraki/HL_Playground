...
|---|---|---|---|---|---|
| 1 | CLOSED 2026-09-10 | PHASE_TWO §6 #5 / §2.1 | §2.1: "The counted figures stay, but as a quiet strip, not as the subject." | hours | ALREADY BUILT — the evidence column was stale. It read "Dashboard.tsx:320-410 is header → greeting → Edit button → QuickLinks → card grid; there is no strip element". `.dsh-strip` is in `features/dashboard/Dashboard.tsx` with a comment arguing §2.1 term by term: tertiary ink on the page ground, no surface, no rule, no elevation, nothing pressable, not arrangeable, and nothing animates because "a figure that moves is a figure a person waits for before they can read it". Seen in the running app: "15,691 rows across 53 tables" beside the greeting. |
| 2 | CLOSED 2026-09-10 | PHASE_TWO §6 #6 / §4 | §4.2: "Options strike through in place when the solver removes them — 200ms." | hours | Closed by `baef440` — the sixth of PHASE_TWO §4's six motions, the only one never built. `.s-refused .s-figure::after` draws the strike in 200ms with `scaleX` from a left origin. Two things about it are load-bearing: the RESTING state is the struck one and the keyframe runs FROM zero, so a refusal never depends on an animation having run; and it is a `border-top` not a `background`, because browsers drop pseudo-element backgrounds when print-background-graphics is off and the struck price would have printed UNSTRUCK on the one artefact a dealer hands a customer. Verified in the app: 8 refused figures, animation `s-strike 0.2s`, `transform matrix(1,0,0,1,0,0)`. |
| 3 | CLOSED 2026-09-10 | UX_PASS §9 #1 · §4.1 | §4.1 — "Provenance is a property of a table, and it is visible. A seeded table wears an `Example` chip on its card, in the nav, and on its module." | hours | Closed by a chip on the table card, verified in the running app: 51 cards on the tables gallery, 51 chips, none missed, reading "BOATS · EXAMPLE". It reads `exampleTableIds` — the same seed-stamp-plus-earliest-`createdAt`-cohort the removal control has used since it was built, so the button that offers to remove them and the cards that are them cannot disagree. THE OTHER TWO SURFACES §4.1 NAMES DO NOT EXIST AS PER-TABLE SURFACES, and that is a finding rather than a shortfall. The nav rail lists "Data 53", not tables. And a MODULE is not example data: it POINTS at tables, and one standing on a mix of seeded and the dealer's own would be claiming a provenance it does not have. §4.1 says it itself — "provenance is a property of a table" — so the table card is where it belongs and where it now is. Neutral, not warning-coloured: example data is how the app teaches itself, not a fault to fix, and amber would spend a state colour on a plain fact. |
| 4 | CLOSED 2026-09-10 | UX_PASS §9 #1 · §4.2 | §4.2 — "One control removes all of it, states the count, and is undoable: 'Remove the 21 example tables and 651 example rows.'" | hours | ALREADY BUILT — the backlog was wrong, and this row is the reason the "verify against the tree" rule exists. `src/features/io/exampleData.ts` shipped it with `exampleData.test.ts` AND a rendering suite, and a session spent an hour re-implementing it in `src/store/` before noticing. That duplicate was deleted. Driven in the running app to confirm rather than trusting the file: Admin → Import / export → footer reads "Remove the 53 example tables" over "And the 15,691 rows in them, and the 9 places they hold up on the dashboard. Tables you have made yourself stay." It is better than §4.2 asked for: the spec wanted tables and rows counted, and it also counts the dashboard places. It is undoable and therefore asks nothing (rule 9), it is drawn ABOVE Clear sheet so the cheap answer is met first, and it is ABSENT rather than disabled when there is no example data — a disabled control owes a reason and the honest one is a sentence about a thing that never happened. |
| 5 | CLOSED 2026-09-10 | UX_PASS §9 #1 · §4.4 | §4.4 — "Presets are neutral, or they are not presets. A `Boats` preset ships the columns every boat has … and *not* `AUS Sailing`." | hours | Closed, and the row named one column of six. The `boat` preset shipped `AUS Sailing` (a named account of ONE business), `Sub Dealer` and `Sub (Exclusive)` (that business's channels), `HO - MU` and `BMT - MU` (its abbreviations for hull-only and boat-motor-trailer markup) and `Matrix` (its word for a code scheme). Somebody in another state making their first Boats table got a pricing band with a stranger's customers in it. The `Markups` section went with the two columns that were in it, because a band with no columns is a heading for nothing. NOT RENAMED TO SOMETHING NEUTRAL, deliberately: a generic `Markup` would be a name `pricing.ts`'s COST_COLUMNS does not know, and that list is what keeps a dealer's buy price off a customer's quotation — inventing a column here would open a hole there. §4.4 says it itself: "brand-specific columns are what the import is for". NEW tables only; the seeded file carries its own columns from the real workbook and is untouched. 17 tests, and they SWEEP every kind rather than asserting six names, because the failure mode is the seventh column added next month. One of them checks no preset draws a band with nothing in it — which is how the orphaned `Markups` heading would have been caught. |
| 6 | CLOSED 2026-09-10 | UX_PASS §9 #2 · §5 | §5 — "Pinning must not be able to author schema … ⚠ These two tables have never been linked. Pinning this motor will create a link table, Boats ↔ Moto | hours | ALREADY BUILT. `BlockCard.tsx:603` gates `ensureJoinTable` behind `linkAsk` — the pin ASKS before it authors, and `acceptLink` is one undo step because "the store collapses everything written in a tick into one history entry, so the table and the pin go back together". The toast names what was made and what it cost: "<name> — a new table, so <source> and <target> can record which goes with which. The sheet now holds N tables." That is §5's rule exactly: a structural change is offered in a sentence that names it, and it is undoable. |
| 7 | CLOSED 2026-09-10 | UX_PASS §9 #2 · §5 | §5 — Finding 18: "Prefer `displayFieldId`; where the guess is weak, say it is a guess." | hours | Closed by `features/rules/obviousColumns.ts`. THE FUNCTION NAME WAS ALREADY RIGHT AND THE BODY WAS NOT: `useDisplayFields` took `fields[0]` — column order, an accident of how somebody's spreadsheet was written — and the button called it "Use the obvious two". Now `displayFieldId` wins where a table nominated one (by id, so a stale id naming a deleted column falls through rather than resolving to whatever now sits at that index), and the first column is offered as what it is. The button drops the word "obvious" the moment either half is a guess, and a sentence under it names the columns before it is pressed — "Model, and Code as a guess: that table has not said which column names a row, so this is the first one." The two halves are said separately because they can differ and a sentence that averaged them would be wrong about both. §7: a suggestion that is confidently wrong is worse than no suggestion. 12 tests, asserting the sentence as hard as the pick. |
| 8 | NEEDS A DECISION | UX_PASS §9 #2 · §5 | §5 — Finding 15: `constraintDefs.ts` records that there is "no per-rule delete by design" | hours | NOT A GAP — A CONTRADICTION BETWEEN TWO DOCS, and not one to settle by writing code. CONFIGURATOR_SPEC.md:341 states it outright: "Rules toggle off, they are never deleted — the experiment is reversible and the authoring survives." `constraintDefs.ts:229` implements exactly that and says so: "The switch. Never a delete: the experiment is reversible and the authoring survives (CONFIGURATOR_SPEC §4b)." So the code is not missing a feature; it is obeying a spec that CLUELESS_USER_TESTS Finding 15 disagrees with. Building `deleteConstraint` would quietly overturn §4b in a commit, which is the "two documents disagreeing" failure CLAUDE.md exists to prevent. THE COST OF THE SPEC AS WRITTEN: a dealer who writes a bad rule can never remove it — only switch it off — so disabled rules accumulate for the life of the sheet, and `clearConstraints` (the only removal) throws away all of them including the good ones. THE COST OF OVERTURNING IT: an authored sentence a person spent time on can be destroyed, and the registry is its own localStorage map so the project store's undo does not cover it — a delete would need its own toast-with-UNDO. A middle answer exists and is also a decision, not a deduction: allow discard ONLY for a rule that is switched off AND has never fired, which keeps §4b's promise about live authoring while letting the dead ones go. ||
| 9 | CLOSED 2026-09-10 | CLUELESS_USER_TESTS §OPEN O5 | 'entity' still reaches the reader from two files nobody on this pass owns. src/types/model.ts:863 - the Match kind's blurb, 'Find the rows of another  | hours | Closed, and the row understated it by eighteen. It named two blurbs in `model.ts`; a sweep found TWENTY reader-facing strings carrying the word, and ELEVEN of them were lint findings — the sentences written to explain a problem to somebody who by definition does not know the vocabulary yet. One was titled "HIDDEN ENTITY". Others: a `#REF?` tooltip, an `aria-label` on the reviewer marks, and a button reading "Extract 'X' entity". All corrected to the dealer's noun. `tools/check-words.mjs` is the durable half and now runs as the sixth guard in `npm test`: twenty assertions would pass for ever and catch nothing, because the failure mode is not "these twenty regress", it is "the twenty-first is written next month by somebody who has not read §6". 423 files swept. It nearly shipped useless: `${entity.name}` renders as "Boats" and the first run reported twelve of those as violations — twelve false alarms out of twenty-one findings, which is the ratio at which somebody turns a guard off. Interpolations are stripped before the words are read, and the sweep refuses to pass if it walked fewer than 120 files, so a broken walk cannot report clean. ||
| 10 | CLOSED 2026-09-10 | CLUELESS_USER_TESTS §OPEN O7 | Nothing dragged is verified. Drag a chip from the palette onto the paper, drag a plate to move it, and drag from one plate's handle to another's were  | hours | Closed by `features/rules/drop.test.ts` — 14 tests over the whole drag contract, which had none. A DataTransfer is four lines to fake, so what is asserted is the DECISIONS `drop.ts` argues for rather than the wiring: the plate lands centred under the cursor and snapped to 16px; a plate never lands on top of another and the walk goes right, right, right, then down and back to the left; that walk is BOUNDED because "a crowded rule gets a plate on top of another rather than a hang" (25 drops on one point all return); and a payload that is not a node kind is refused rather than trusted. THE ONE A RENDERING TEST WOULD HAVE MISSED: a foreign drop returns null WITHOUT calling preventDefault, so somebody who drags a file onto the canvas by accident gets the browser's own behaviour instead of silence. TWO EXPECTATIONS WERE WRONG AND THE CODE WAS RIGHT, both now asserted: a rule from `createRule` is never node-empty (it is seeded with a start plate), so `nextNodePosition`'s 64,64 fallback is unreachable through the normal door; and that seed sits at 60,60, OFF the 16px grid, so the next plate lands at 384,64 rather than 380,60 — the code corrects the seed instead of inheriting its offset and carrying it along the whole row. ||
| 11 | CLOSED 2026-09-10 | CLUELESS_USER_TESTS §OPEN O9 | The rules rail's rename and delete buttons are hover-only hit targets... they are pointer-events: none until the row is hovered or focused. Correct fo | hours | ALREADY ANSWERED, and the answer is better than the row asked for. `rules.css:1275` carries an `@media (hover: none)` block that makes the acts live on the ACTIVE row and grows the targets to 28px; `RulesList.tsx:138` sets `is-active`, so a tap opens a rule and its controls arrive with it. Three things in that block are argued rather than assumed. `hover: none` and NOT `pointer: coarse`, because "a touch laptop still has a mouse, still hovers, and needs none of this". A `pointer-events` transition-delay was written to gate a mis-aimed thumb and then MEASURED not to gate, because `:focus-within` makes the cluster live on the same tap — and it was removed rather than left claiming a protection it does not give. And the real answer is rule 9: `deleteRule` records an undo step, so two taps produce "Deleted 'Motor fitment — Highfield' — the flow drawn on it went with it" with UNDO beside it. O9 reasoned in an app that had no undo. The worst case of a mis-aimed thumb is one tap back, not a lost afternoon. ||
| 12 | CLOSED 2026-09-10 | MODULE_SYSTEM+TENANCY MODULE_SYSTEM §2 (defe | §2 defect 2 — "`deleteEntity` does not cascade into views. Delete a table and its module would point at nothing." | hours | Closed by `src/store/deleteCascade.ts`, a pure function so the confirm can price the act without performing it. Four decisions, each argued: a page whose `rootTableId` is gone is DELETED (a page is what it is about, not a reference it can lose); a block whose table is gone is dropped and the page survives, recursing to the contract depth of 3; a module loses the table from `tableIds` and survives on what is left, INCLUDING when it loses `[0]`; a module with none left is deleted. The one dangling pointer with a defined resting place is `viewId`, which is cleared rather than deleting the module — model.ts already blesses that state: "Absent means the module lists but does not open." `cascadeSay` turns the same result into §7's computed blast radius, and the confirm sheet's copy was rewritten because this change made it FALSE — it read "deleting the table does not delete it" about a page, which was true and awful. Measured in the running app on Formosa: "This page is about Formosa and goes with it", "4 pages elsewhere draw a block from this table and lose it", "This module loses it and lists what it has left — Boats". The sheet also promises Ctrl+Z brings them back; `undo.test.ts` now asserts that round trip rather than trusting that `record()` runs before `mutate()`. 16 + 1 tests. |
| 13 | CLOSED 2026-09-10 | MODULE_SYSTEM+TENANCY TENANCY §4.6 | TENANCY §4.6 — "`roles` are never written. `ProjectExport.roles` is declared in the contract with a paragraph explaining why grants are unreadable wit | hours | Closed, and it was THREE ends rather than the two the row named. The row said roles are never written and never read; the third was that `validateEnvelope` did not know the key either, so writing them without teaching the validator would have exported roles and dropped them silently at the door — the exact "silence under a button labelled Everything" that `envelope.design.test.ts` was written to prevent. `exportPayload` now writes them sorted by `createdAt` like every other collection, so two exports of one unchanged sheet stay byte-identical. `validateEnvelope` collects them into `seenIds` — a role id keys a store record, the same reason a module id does, and two under one id would make the second edit silently move the first. `restoreDesign` puts them back BEFORE the modules that name them, and an existing role wins on a merge: a job at this dealership does not become a second job because a backup mentions it. That is the one place a merge must not reissue an id, since reissuing would leave the grants pointing at the old one. 7 tests asserting what a person gets back rather than that a key is present. `orphanRoleIds` had existed all along to find this case and the settings panel said so in words — it was reporting a fault the exporter created. ||
| 14 | CLOSED 2026-09-10 | MODULE_SYSTEM+TENANCY TENANCY §4.6 | TENANCY §4.6 — "`OrgProfile.createdAt` cannot survive a replace… the store stamps `createdAt: nowIso()` on the profile it builds (`useProjectStore.ts: | hours | Closed, and the row named half the bug. `setOrganisation` stamped `createdAt: nowIso()` unconditionally, so every call reset it — but the reason that mattered is the caller: `Shell.tsx:718` calls it after EVERY project swap to put the business back on the masthead. The one effect written to SURVIVE a swap was itself re-dating the business to the moment of the swap, so loading a prepared set quietly moved the date to today. `knownOrg` had held the whole `OrgProfile` — `createdAt` included — all along, and the call dropped it on the floor. Both halves fixed: the store takes an optional `createdAt` and otherwise keeps what is already on the sheet, and the Shell hands back the whole profile it remembered. A RENAME KEEPS THE DATE, deliberately: changing what a business is called does not change when it started, and this app holds one organisation per sheet so there is no reading where a new name is a new organisation. Four tests asserting when the business started — the fact the field is for — rather than the shape of the object holding it. ||
| 15 | CLOSED 2026-09-10 | CONFIG_FINDINGS+QUOTE_FINDINGS PERF §Fixes 1 (follow- | Implied by Fix 1 — one plate tier is enough because "nothing legible is lost". | hours | ALREADY IN THE STATE THE ROW ASKS FOR. The item is that ONE plate tier is enough because "nothing legible is lost", and `tableLod.ts` has exactly one plate band and no second tier — `PLATE_ZOOM_IN` 0.6 and `PLATE_ZOOM_OUT` 0.66. The two numbers are hysteresis, not two tiers: "a wheel gesture crosses the threshold many times as it passes", and one threshold would flicker. Nothing to build. |
...
| 17 | CLOSED 2026-09-10 | BOARD_CUSTOMISATION+SALES_BOARD CONFIGURATOR §B | §B item 4 — the share, where a share is a fact (`on 3 of 7`, never `on 1 of 1`). | hours | Closed at `measuredRate`, which is where the row said it was: `if (held >= tested)` printed "holds on 1 of 1 pairings, no exceptions" — true, and not evidence. A rule that has met one case has not been measured against anything, and giving that the same sentence shape as "holds on 3,890 of 4,017" is the app lending its own credibility to a figure that has not earned it. It is now silent below two, and NOTHING IS SAID INSTEAD: a narrowing whose rate is not worth stating carries its reason alone, because `curationChipParts` pushes `measured` only when it is there. Nothing in the seed moves — the smallest measure in `RULE_LEDGER` is 530 — so this guards the rule somebody writes next month against the one table with one row in it. AND A SECOND FAULT CAME OUT WITH IT: the rate printed its figures UNGROUPED while the chip beside it groups. "10 of 434 Trailers" over "holds on 7830 of 7830 remote-helm cells" is one reading written two ways, which is the disagreement this file exists to prevent, in punctuation instead of arithmetic. Found because a test expectation was typed grouped without thinking — a reader's instinct being right. Both grouped now, asserted on the real ledger figures. 6 tests. |
...
| 24 | CLOSED 2026-09-10 | MODULE_SYSTEM+TENANCY MODULE_SYSTEM §5 | §5 — "'Define what can be done within that module' is ten switches on the module" (browse, search, open, add, edit, delete, relate, quote, export, imp | hours | Closed by the same commit that made the travel verbs real. §5's list is browse, search, open, add, edit, delete, relate, quote, export and import, and `ModuleCapability` now carries all ten in that order — `import` was the one missing, and it was missing at every end: no type member, no label, no switch, no column in the access grid. It is there because `MODULE_CAPABILITIES` is what every one of those surfaces iterates, which is the property `DESIGNER_CAPABILITIES` was built to keep. §5's own words for it — "Rows arrive from a file into the master table" — are what it does: the file lands in `travel.from`, the same table `CatalogWrites.into` puts a new row into, so the file a person brings back and the row they add cannot be about two different registers. THE ELEVENTH SWITCH IS STILL OUTSIDE THE CONTRACT and that is row 25's, not this one's: `configure` is drawn from `ruleCapability.ts`, a registry mirror, and it is spliced into the designer's list rather than the type. Seen in the running app on Highfield: eleven switches, ten of them the contract's. |
...
| 29 | CLOSED 2026-09-10 | CONFIG_FINDINGS+QUOTE_FINDINGS UX_REWORK §2 | Clause rows are mono 10.5px. | hours | The row's own subject was ALREADY CORRECTED — every plate text class in `rule-nodes.css` is 11px, twelve of them, none below. What closing it properly turned up is the reason a sweep beats an assertion: `.ds-chip` in `src/styles/ds.css` was still 10.5px, and that is the DESIGN SYSTEM'S OWN chip — drawn only in the gallery today, which is exactly what made it matter, because a gallery is where the next screen copies its chip from. It is 11px now, and it was the last one in shipped CSS. THE DURABLE HALF: `check-styles.mjs` now fails on any px font-size under 11 anywhere in `src`, so rule 2 has a guard for the first time — it was kept by hand for a year and then was not. `src/design/` is exempt and it is the only exemption: the gallery draws MINIATURES of whole screens at 5.8px, which is a picture of type rather than type a person reads. `rem` and `em` are not checked, because a guard that guessed at the root size would be inventing the number it fails on. Proved to bite: put 10.5 back and it names `src/styles/ds.css:849` and exits 1. |
...
| 32 | CLOSED 2026-09-11 | CONFIG_FINDINGS+QUOTE_FINDINGS CONFIG §4 #3 | Adopt 3 — Empty means unrestricted, applied everywhere, and said in the UI: "Empty list = no filter." | hours | Closed, and the answer is that this app has TWO engines that read the same-shaped filter in OPPOSITE directions, and both are right. `views/filter.ts:67` skips an empty `values` list, so a curated list with nothing in it admits everything — that IS Adopt 3, and hl-admin's own reason for it is that a fresh list showing nothing would hide a dealer's whole catalogue behind a setting they have never opened. `table/core/view.ts:217` admits NOTHING through an empty one, which is Excel's uncheck-everything in a surface that imitates a spreadsheet on purpose. THE DIFFERENCE IS WHO WROTE THE EMPTINESS: in the register a person opened a menu and unticked every box, and in a curated list nobody has curated yet. Nothing was changed — the register's "Brand: none" chip and its rail summary are TRUE of the register, so the row's "said in the UI" half is already kept at every surface where it applies, and `BlockCard` cannot even create the empty state (unticking the last value drops the filter object, so the `continue` guards a filter arriving from an import). WHAT WAS MISSING WAS THE RECORD. The two sites named neither each other nor the ruling, so the next person to find one while working on the other would "fix" the inconsistency and break a catalogue or a register. Both now carry the argument and point at each other, and `views/emptyFilter.test.ts` asserts the two side by side in ONE file — the failure mode is not that either regresses alone, it is that somebody reconciles them. 6 tests. |
| 33 | CLOSED 2026-09-11 | CONFIG_FINDINGS+QUOTE_FINDINGS CONFIG §4 #9 | Adopt 9 — Quarantine, don't coerce: any value starting with `#` is rejected; cached `#N/A`/`#VALUE!` cells are reported, never imported as prices. | hours | Closed at `coerceCellText`, which is the ONE seam every file and every paste comes through — so one guard covers the CSV door, the re-upload and the clipboard rather than three. The refusal already held on a price column (`#N/A` is not a number) and the hole was the TEXT column, where a cached error sailed through as the literal string and then sits on a catalogue face in front of a customer, sorts, exports, and eventually gets a price typed beside it. NOT "ANY VALUE STARTING WITH #", which is another codebase's wording and would eat "#1 Best Seller" and every "#4 rigging kit" a dealer has typed: the list is the ten tokens Excel actually writes, which are not ambiguous at all. `#REF?` with a question mark is OURS (`GridCell.tsx:182`, drawn for a dead link, never stored) and is deliberately not in it. THE GRID IS UNTOUCHED — it does not use this seam, so a person may still type whatever they mean into their own cell; this guards what ARRIVES. AND THE SECOND HALF, "reported": `exceptionsFor` returned early on text because nothing could fail to be text, so the reading would have promised "0 cells will be empty" over a create that emptied one — the count disagreeing with the act. Text now reports them in the same shape the number and list branches use. Driven through the real door with a supplier file carrying both: "1 is a spreadsheet error rather than a value, and will land empty", the token itself drawn as a chip beside it, and the footer "Makes hash with 2 columns and 4 rows. 2 cells do not fit the types chosen and will be empty" — while "#1 Best Seller" arrives untouched. 8 + 4 tests. |
| 34 | CLOSED 2026-09-10 | UX_PASS §9 #4 | Propose modules on the empty dashboard (§8) | ~1 day | Closed, and the row named ONE of the two empty dashboards. `proposals.ts` and its 20 tests had been shipping the reading for weeks and the FRONT DOOR was the only thing asking it; the MODULES SCREEN — the one a person presses when they have gone looking for their places — still drew a definition of the word "module", a count of the tables, and a button that opened a blank panel. Every word of that is true and none of it is an answer. So the list moved out of `CardBody` into `dashboard/ProposeList.tsx` and the modules screen draws the same one: same reading, same Row, same seed, same three-click panel. IT IS ONE SURFACE WITH TWO CALLERS, not two lists that agree today. Driven in the app on a blank sheet with one table made through the real door: "From your 1 table, these look like places in your business" over "Highfield Inflatables · 1 table · 0 rows", and pressing it opens New module with that table already picked. THREE THINGS THE BUILD CHANGED FROM THE SKETCH, each measured. (1) The count line TURNS ROUND rather than disappearing — "You have 3 tables and no modules" ends on what is missing, and the same figure introduces what is there. (2) It stays a SENTENCE: `SectionHead` uppercases, so §8's own line came out as "FROM YOUR 1 TABLE, THESE LOOK LIKE PLACES IN YOUR BUSINESS" — a shouted line with a value in it, which rule 3 forbids — so `lead` became optional and the caller that has already said what the list is does not say it twice. (3) `[ Create all three ]` IS NOT BUILT, and the reason is `useProjectStore.ts:115`: "Views and modules stay out" of the undo stack. It would be the only irreversible multi-write on any empty state in this app, pressed by the person who has been using it for ninety seconds. Each proposal opens the panel it is a seed for; the escape hatch stays and stops calling itself New module, because the proposals above it ARE the new module. 10 rendering tests over the decisions, not the shape. |
| 35 | CLOSED 2026-09-10 | MODULE_SYSTEM+TENANCY MODULE_SYSTEM §10 Phas | §10 Phase 4 — "`export`/`import` as real module capabilities" | ~1 day | Closed, and it was the same fault `writeCaps.ts` fixed one wave earlier: a switch that changed nothing. `export` had been in the contract since the module system landed — a switch in the designer, a column in the access grid — and the note beside it SAID "Taking a copy of this list out is not built yet". `import` was not in `ModuleCapability` at all, so an administrator could grant the right to take a price list out and had no way to say whether one may be pushed back over it. NOTHING NEW PERFORMS THEM: `io/tableCsv.ts` and `io/TableRoundTrip.tsx` have shipped the whole round trip for months — a file of the register, and a preflight that reads one back, writes nothing, and says in counts what it would change. `travelCaps.ts` decides whether the catalogue may put those controls on the page bar, and `CatalogTravel.tsx` mounts them; neither becomes a second exporter with its own rules about what leaves a dealership. THE TWO ARE SEPARATE SWITCHES because they are opposite risks — one leaks a cost column, the other overwrites six hundred rows — and `import` publishes BOTH doors rows arrive through (Re-upload and Paste rows), because an admin who withheld the file door did not agree to the clipboard one. THE REFUSAL THIS FILE OWNS: a module drawing more than one register refuses in words rather than writing a file of `tableIds[0]` and calling it the module's — the confidently-wrong control §7 rates worse than none. The withheld verbs JOIN the write verbs in one sentence rather than starting a second apology, which is `accessSay.ts`'s lesson, so `withheldSay` now takes any capability. And the switch names WHAT LEAVES: "every column the table holds, cost columns included", because granting this grants the cost structure and finding that out in Excel is finding it out too late. Driven end to end on the real sheet: Highfield → Settings → Export and Import on → Catalog → the bar carries Export, Re-upload and Paste rows → "Highfield Inflatables 2026-09-10.csv — 588 rows and 33 columns. Edit it in Excel, then press Re-upload. Image Link is in it to read, not to edit." 12 tests on the reading, 6 on what the page publishes. |
| 36 | CLOSED 2026-09-10 | MODULE_SYSTEM+TENANCY TENANCY §4.1 | TENANCY §4.1 — "One tenant key, and it is the slug. `OrgProfile` gains `slug: string`, and `orgKeyOf` returns it rather than the lowercased name. Unti | ~1 day | Closed at all four sites §4.1 names. `OrgProfile` gains `slug?: string`, minted ONCE from the first name and kept — the same rule as `createdAt` beside it, and for the same reason: a rename is a rename, not a new business. `orgSlug` is for minting only, never re-derived, because a function that recomputed it on every read would be the bug it exists to fix. `orgKeyOf` returns the slug and IGNORES the name once there is one. The name fallback stays and is the migration's other half rather than a hedge: a sheet saved before slugs has none until `setOrganisation` runs again, and its rules must keep resolving meanwhile. `adoptSlugKey` moves them across the first time it can see both keys at once, runs at boot in `App.tsx`, and is idempotent by construction because it takes the old key away. Nothing is overwritten — a rule already under the slug wins, since the slug key is what everything writes to now. 14 tests, and the one that matters goes through the real door: onboarding sets the organisation, a rule is written, the business corrects its own name, and the rule is still there. `getConstraints()` takes no key — it reads the LIVE store — so a test that passed one would have been asserting nothing, which the first draft did. ||
...
| 86 | CLOSED 2026-09-10 | FOUND 2026-09-10, verifying row 3 | Two counted figures disagree about the same tables. The removal control says "Remove the **53** example tables"; the tables gallery header says "**51** Tables" and the nav rail says "Data 53". Every figure in this app is counted rather than typed, which is the whole point of it, so two of them differing by two is either a filter nobody stated or a count that is wrong. | hours | NEITHER FIGURE WAS WRONG AND NEITHER SAID WHAT IT COUNTED. Measured in the running app: nav 53, gallery header 51 Tables / 15,651 rows, removal control 53 tables / 15,691 rows. `HomeStage`'s `groups` filters `!isRetired(e)`, so the header counts LIVE tables — correct for a gallery of what a dealer sells — and the other two count what is on the sheet, correct for a delete. Two tables are retired and hold 40 rows between them. The row also understated it: the ROWS figure disagreed too, by exactly those 40, and nobody had noticed because the tables gap was the visible one. Closed by one computed sentence under the figure it reconciles: "2 retired tables and their 40 rows are not counted here". 51 + 2 = 53 and 15,651 + 40 = 15,691, both closable by a person on the screen. Absent when there is nothing to say, because "0 retired" is a sentence about a thing that never happened. The precedent is on the cell above it — a term that "read as an overclaim the moment the strip drew the split, so the term is corrected to what is counted". ||
