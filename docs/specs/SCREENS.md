...
|---|---|---|---|---|
| `home` — the front door | a dealer, forty times a day | see what they sell, who for, what is out; raise a quote | greeting, four kinds photographed, the brands as a shelf, open quotes as cards — one primary act, "New quote" | **done** — unframed, hero step, brand shelf and foot panes without boxes; then the tile rule: a kind or a brand with a photograph is a scene tile (Nimbus's — the picture to the edge, the name in the wide light face over it, an outline pill), one with a render is a studio tile (Zodiac's — the render on white, the name under it); a kind with neither shows its plate, and no picture is invented |
| `start` — the quote picker | a dealer starting a quote | choose the place (brand) to quote from | Nimbus's tile where a place has a photograph (the picture fills the card, the name over it in the light wide face, Quote as an outline pill), Zodiac's where it has a render (large on white, the name under it) — read from the picture's pixels | **done** — tiles, the night the standard changed |
| `start` at a place — the place screen | same, one step in | choose the model and finish | the same two tiles per model, the model named not the SKU; finishes in the bar; a floating action card, never a solid bar | **done** — tiles, the night the standard changed |
| `quote` (draft) — the configurator | a dealer, with a customer beside them | build the rig: hull, motor, trailer, dealer fit, who it is for | **chapters, after Saxdor**: one chapter per decision, each the height of the window; a full-bleed photograph when the boat has one, the render large on white when it does not (`scene.ts` reads the pixels); the name at the wide light face; one floating card of choices; the finishes as a swatch grid that changes the boat; a foot pill with where you are and the price; a finale over the boat | **done** — the night the standard changed; sweep 8.70× |
| `quote` (issued) — the document | the dealer, then the customer at home | read what was offered; print it | **Porsche's PDF**, with the chapters' cover: on screen the cover is the boat's photograph where it has one (the name over it in the wide light face, the chips on the picture), the white cover where it has a render; on paper always the white cover; the price beside who it is for, sections as tables from page two | **done** — `91d5cc9`, then the cover the night the standard changed |
| `table` in the catalogue lens | a dealer showing a range | browse one brand's models | masthead, hairline bays, hulls at 360–520px on white, one-baseline captions; each model's card takes the tile rule — a photographed model is a scene tile, a rendered one a studio tile, decided from the picture's own pixels | **done** — then the tile rule the night the standard changed |
...
| `module` — a place's workspace (Boats, Highfield, Yamaha…) | a dealer working in one place | see what the place holds, open its catalogue, its quotes, its price columns, its set-up | the tile rule at page scale: where the place's own rows hold a photograph the header is that photograph (`doorPicture`'s, a real row's) with the name over it in the wide light face; where they hold renders it is the name on white — Boats opens on a Stacer, Motors on white. Five tabs under a hairline with the rule in the action colour. The dashboard is sections, not boxes: the range as doors that take the tile rule, recent quotes and activity beside it, the siblings and what goes with them as outline pills. The Catalog tab's tiles take the rule too; Pricing is one register of levels — the place as a drawer line, a row per level with the column it reads in mono, what it prices, and the one on a quote's face — with the rules it goes by as hairline sections beneath, a derivation a row with its switch at the right | **recut** 2026-09-16 — Settings the same day: one column of sections under hairline heads, a verb a row with its switch at the right, refusals as sentences beside a rail, the tab's own head clipped because the page header is the name. Quotes the same day: the register's own shape — what they come to, then reference · customer · what it is for · day · where it is · comes to as hairline rows; with none raised it says so and offers New quote. `tools/shot-module.mjs` photographs every tab; `tools/shot-mquotes.mjs` raises one quote through the app and photographs the tab with it |
...
| `data` — the register of tables | a dealer or the owner | see the shape of the business: 53 tables, what each holds, how many rows | the one drawing every register now shares: nothing boxed, a heading row of 11px labels on white over a strong rule, rows on the 32px ladder, figures in mono, the kind's glyph a glyph in its hue, the five doors as outline pills, the sort rule and the focus ring in the action colour | **done** — `aa88209`; recut 2026-09-16 to the registers' drawing, 18 rows at 1280×800 measured |
| `places` — Modules, the register of places | the owner, setting the shop up | see every place (a brand, a kind, a custom table) and open its settings | the places as tiles under kind headings, chips to narrow; each tile takes the tile rule — a place with a yard shot is a scene tile, its mark top-left in the scene's ink; one with a render is a studio tile; one with no picture shows its mark alone | **done** — the tile rule the night the standard changed |
...
| `table` — the register of rows | a dealer, all day | the sheet: search, sort, fill, row commands | one name (the bar keeps the way back; the masthead is a line — kind, name, count — with the lens switch beside it and the search and filters on one row under it); rows on a 28/36 ladder with the drawer lines on the same ladder; no vertical rules on data cells, the frozen identity column keeps its one edge; 11px labels on a white heading row; a drawer's name in sentence case, its count a mono figure; the active ring and the selection wash in the action colour | **recut** 2026-09-16 — measured at 1280×800: 9 data rows on Highfield's sheet, from 5. Eighteen is not reachable on a table whose own hierarchy (series ▸ model) costs two lines per four rows; on a one-level table the same chrome gives 17–18. `tools/shot-sheet.mjs` measures it |
