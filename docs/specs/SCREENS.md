...
| `module` — a place's workspace (Boats, Highfield, Yamaha…) | a dealer working in one place | see what the place holds, open its catalogue, its quotes, its price columns, its set-up | the tile rule at page scale: where the place's own rows hold a photograph the header is that photograph (`doorPicture`'s, a real row's) with the name over it in the wide light face; where they hold renders it is the name on white — Boats opens on a Stacer, Motors on white. Five tabs under a hairline with the rule in the action colour. The dashboard is sections, not boxes: the range as doors that take the tile rule, recent quotes and activity beside it, the siblings and what goes with them as outline pills. The Catalog tab's tiles take the rule too; Pricing is hairline sections | **recut** 2026-09-16 — Settings the same day: one column of sections under hairline heads, a verb a row with its switch at the right, refusals as sentences beside a rail, the tab's own head clipped because the page header is the name. Quotes the same day: the register's own shape — what they come to, then reference · customer · what it is for · day · where it is · comes to as hairline rows; with none raised it says so and offers New quote. `tools/shot-module.mjs` photographs every tab; `tools/shot-mquotes.mjs` raises one quote through the app and photographs the tab with it |
